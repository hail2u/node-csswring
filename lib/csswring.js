...
var _reColorHSLAFunction = /(^|\s|\(|,)(hsla\s*\(.*?\))/gi;
...
var _reColorFunction = /(^|\s|\(|,)((?:rgb|hsl)\s*\(\s*(?:[\d.]+%?(?:\s*,\s*|\s*\))){3})/gi;
var _reColorHex = /(^|\s|\(|,)#([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])/gi;
var _reUrl = /(^|\s|\(|,)url\s*\(\s*(.*?)\s*\)/gi;
...
var _isIdentifier = function (str) {
  return _reIdentifier.test(str);
};
...
var _unquoteFontFamily = function (family) {
  ...
  if (list.space(family).some(_isIdentifier)) {
    family = '"' + family + '"';
  }
...
// Convert `hsla()` notation to `rgba()` notation
var _toRGBAColor = function (m, leading, color) {
  return leading + onecolor(color).cssa();
};

// Convert `rgb()` or `hsl()` notation to hexadecimal notation
var _toHexColor = function (m, leading, color) {
  return leading + onecolor(color).hex();
};
...
var _wringValue = function (value) {
  ...
  value = value.replace(_reColorHSLAFunction, _toRGBAColor);
  ...
  value = value.replace(_reColorFunction, _toHexColor);
...
var _wringAtRule = function (atRule) {
  ...
  if (!atRule.params || params.indexOf('(') === 0 || params.indexOf('"') === 0) {
    atRule.afterName = '';
  }
};
