...
var removeUnitOfZero = function (m, leading, num, unit) {
  if (
    this === "flex" ||
    this === "-ms-flex" ||
    this === "-webkit-flex" ||
    this === "flex-basis" ||
    this === "-webkit-flex-basis"
  ) {
    return leading + num + unit;
  }
...
var wringDecl = function (preserveHacks, decl) {
  ...
  value = values.map(wringValue.bind(prop)).join(",");
...
module.exports = postcss.plugin(pkg.name, function (opts) {
  ...
  return function (css) {
    css.semicolon = false;
    css.after = "";
    css.eachComment(wringComment.bind(null, opts.removeAllComments));
    css.eachDecl(wringDecl.bind(null, opts.preserveHacks));
