...
var _reColorFunction = /(^|\s|\()((?:rgb|hsl)\s*\(\s*(?:[\d.]+%?(?:\s*,\s*|\s*\))){3})/g;
var _reColorHex = /(^|\s|\()#([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])/g;
var _reUrl = /url\s*\(\s*(.*?)\s*\)/g;
...
var _cleanFamilies = function (value) {
  var families = list.comma(value);
  families.forEach(function (family, index) {
    family = family.replace(_reQuotedString, '$2');

    if (list.space(family).some(function (identifier) {
      return _reIdentifier.test(identifier);
    })) {
      family = '"' + family + '"';
    }
...
// Convert `rgb()` or `hsl()` notation to HEX color
var _toHex = function (m, space, color) {
  return space + onecolor(color).hex();
};

// Use 3-digits HEX color if possible
...
// Unquote `url()` notation if possible
var _unquoteUrl = function (m, url) {
...
var _wringValue = function (value, prop, decl) {
  if (prop === 'content') {
    return value;
  }

  if (prop === 'font-family') {
    return _cleanFamilies(value);
  }
  ...
  value = value.replace(_reColorFunction, _toHex);
  value = value.replace(_reColorHex, _to3DigitHex);
  value = value.replace(_reUrl, _unquoteUrl);
...
var _wringDecl = function (decl) {
  ...
  decl.value = _wringValue(decl.value, decl.prop, decl);
  ...
  if (
    (_reDescriptorFontFace.test(decl.prop) && decl.value === 'normal') ||
    (decl.prop + decl.value === 'font-weight400')
  ) {
    decl.removeSelf();
  }
};
...
var _wringAtRule = function (atRule) {
  ...
  if (atRule.name === 'media' && atRule.rules.length === 0) {
    atRule.removeSelf();
  ...
  if (atRule.name === 'import') {
    params = params.replace(_reUrl, '$1');
...
CSSWring.prototype.postcss = function (css) {
  ...
  if (css.last.type === 'atrule' && css.last.name === 'import') {
    css.last.semicolon = true;
  }
