...
var _reDescFontFace = /^font-(style|stretch|variant|feature-settings)$/i;
var _reWhiteSpaces = /\s+/g;
var _reValueLeadingZeros = /(^|\s|\()0+([1-9]\d*(\.\d+)?)/g;
var _reValueZero = /(^|\s|\()(0)(%|em|ex|ch|rem|vw|vh|vmin|vmax|cm|mm|in|pt|pc|px)/gi;
var _reValueDecimalWithZero = /(^|\s|\()(-)?0*([1-9]\d*)?\.(\d*[1-9])0*/g;
...
var _wringValue = function (value, prop, decl) {
  ...
  value = value.replace(_reValueLeadingZeros, '$1$2');
  value = value.replace(_reValueZero, '$1$2');
  value = value.replace(_reValueDecimalWithZero, '$1$2$3.$4');
...
var _wringDecl = function (decl) {
  ...
  if (
    (_reDescFontFace.test(decl.prop) && decl.value === 'normal') ||
