...
var reDecimalWithZero = /(^|\s|\(|,)(-)?0*([1-9]\d*)?\.(\d*[1-9])0*/g;
var reDescriptorFontFace = /^font-(style|stretch|variant|feature-settings)$/i;
var reLeadingZeros = /(^|\s|\(|,)0+([1-9]\d*(\.\d+)?)/g;
...
// Filter optional `@font-face` descriptor
var filterOptionalFontFaceDescriptor = function (decl, index) {
  var prop = decl.prop;
...
var removeDefaultFontFaceDescriptor = function (decl, index) {
  ...
  if (
    ...
    (prop === 'unicode-range' && /u\+0{1,6}-10ffff/i.test(value)) ||
...
var wringValue = function (value) {
  ...
  value = value.replace(/\s([),])/g, '$1');
  value = value.replace(reLeadingZeros, '$1$2');
  value = value.replace(reZeroValueUnit, '$1$2');
  value = value.replace(reDecimalWithZero, '$1$2$3.$4');
...
var wringRule = function (rule) {
  ...
  decls = {};
  rule.each(removeDuplicateDeclaration);
};
...
var wringAtRule = function (atRule) {
  ...
  if (atRule.name === 'font-face') {
    if (atRule.decls.filter(filterOptionalFontFaceDescriptor).length < 2) {
      atRule.removeSelf();
...
CSSWring.prototype.postcss = function (css) {
  ...
  css.eachRule(wringRule.bind(this));
  css.eachAtRule(wringAtRule.bind(this));
