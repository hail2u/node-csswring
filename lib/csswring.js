...
var uniqueSortedArray = function (array) {
  var result = [];
  var seen = '';
...
var wringRule = function (rule) {
  ...
  var selectors = rule.selectors.map(wringSelector).sort();
  ...
  rule.selector = uniqueSortedArray(selectors).join(',');
  var decls = {};
  rule.each(removeDuplicateDeclaration.bind(decls));
};

// Collect `@import`
var collectImport = function (atRule) {
...
CSSWring.prototype.postcss = function (css) {
  ...
  var imports = [];
  css.eachAtRule(collectImport.bind(imports));
  var first = getFirstRule(css.rules);

  for (var i = 0, l = imports.length; i < l; i++) {
    if (first) {
      css.insertBefore(first, imports[i]);
    } else {
      css.append(imports[i]);
    }
  }
