...
var _reColorFunction = /(^|\s|\(|,)((?:rgb|hsl)a?\(.*?\))/gi;
...
var _filterOptionalFontFaceDescriptor = function (decl, index) {
  var prop = decl.prop;
...
var _wringAtRule = function (atRule) {
  ...
  if (atRule.name === 'font-face') {
    if (atRule.decls.filter(_filterOptionalFontFaceDescriptor).length < 2) {
      atRule.removeSelf();
