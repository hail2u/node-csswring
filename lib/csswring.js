...
// PostCSS Processor
csswring.processor = function (conf) {
  _preserveHacks = false;

  if (conf && conf.preserveHacks) {
    _preserveHacks = true;
  }

  _removeAllComments = false;

  if (conf && conf.removeAllComments) {
    _removeAllComments = true;
  }
...
// Wring CSS codes
csswring.wring = function (css, opts, conf) {
  return postcss().use(csswring.processor(conf)).process(css, opts);
};
